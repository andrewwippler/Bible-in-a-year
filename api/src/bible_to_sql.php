...
function getFormattedReferences($array)
{
    $newArray=[];
    $err = [];
    $book = '';
    foreach($array as $k => $a) {
        // Psalm 34, 56, 84, 1 Cor 3
        // always capture book
        if (preg_match('/^\w*[^:]*/', $a) && preg_match('/[a-zA-Z]*/', $a)) {
            $string = preg_replace('/\s(\S*)$/', '|$1', trim($a)); 
            $bookAndChapter = explode("|", $string);
            if (!preg_match('/.*\d.*/', $bookAndChapter[0])) {
                $book = $bookAndChapter[0];
                $err[][$k] = "Book: $book";
            }
        }
        // just a number, apply previous book
        if (!preg_match('/.*[a-zA-Z].*/', $a) && !preg_match('/.*(:|-).*/', $a)) {
            $chapter = preg_replace('/\s+/', ' ', "$book $a");
            $newArray[] = $chapter;
            $err[][$k] = "In Chapter: $book $chapter";
            continue; // prevent the below from happening
        }
        // Psalm 34-39
        if (preg_match('/^\w*[^:]*-[^:]*/', $a) && preg_match('/\w*-\d*/', $a)) {
            $string = preg_replace('/\s(\S*)$/', '|$1', trim($a)); 
            $justChapters = explode("|", $string);
            $book = $justChapters[0];
            $chapSplit = explode("-", $justChapters[1]);

            // <= because we want the last value too.
            for ($i=$chapSplit[0]; $i <= $chapSplit[1]; $i++) { 
                $chapter = preg_replace('/\s+/', ' ', "$book $i");
                $newArray[] = $chapter;
            }
            continue;  // prevent the below from happening
        }
